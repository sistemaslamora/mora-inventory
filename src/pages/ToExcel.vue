...
export default defineComponent({
  ...
  setup () {
    ...
    const verificarExcel = async (data: any) => {
        let info = []
        data.forEach(async (element) => {
            ...
            if(resultAvatar.success === 0) {
              setTimeout( () => {
                info.push(element)
                console.log('resultado success',resultAvatar)
                console.log('la data',element)
              },1000)
            }
    ...
    const subirExcel = async() =>{
        try {
             ...
             items.value = await readXlsFile(files.value)
             verificarExcel(items.value)
    ...
    const setTemplate = async() => {
     ...
     //   const ws = XLSX.utils.json_to_sheet([]);
        ...
        const heading  = [row1, row2, row3, row4, row5, row6, row7, row8, row9, row10]
